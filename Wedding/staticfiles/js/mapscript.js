$(document).ready(function () {
    /* THIS IS NOT A PART OF THE MAP'S SCRIPT! Don't put it on your site!
     ...
     */(function () {
        ...
        n.stringify = function (e) {
            ...
            var r = {type: e.type, content: n.stringify(e.content), tag: "span", classes: ["token", e.type], attributes: {}};
    ...
    $(function ($) {
        var msA = $('#maps-switcher').find('a'), scroll_timer, displayed = false, $message = $('#top-link'), $window = $(window), top = $(document.body).children(0).position().top;
        $('.in-nav').click(function () {
            var elementClicked = $(this).attr("href"), destination = $(elementClicked).offset().top;
...
//        De aici shchimi marimea la harta pe care o folosesti, modifici doar numarul la size in functie de cele 8 imagini de harti din directorul cssmap-romania
//        de exemplu:
//        $("#map-romania2").cssMap({size: 1280, tooltips: false, cities: true});
        
        $("#map-romania").cssMap({size: 960, tooltips: "floating"});
        $("#map-romania2").cssMap({size: 540, tooltips: false, cities: true});
        $("#map-romania3").cssMap({size: 430, multipleClick: true, clicksLimit: 3, searchLink: 'Căutare'});
        $("#map-romania4").cssMap({size: 1280, tooltips: "floating-bottom-right", visibleList: false, cities: true});
        $("#map-romania5").cssMap({size: 430, onClick: function (e) {
                var a = e.children('a'), link = a.attr('href'), text = a.text(), className = e.attr('class').split(' ')[0];
